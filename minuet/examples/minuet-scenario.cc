...
void GerarValoresAleatorios() {
    ...
    std::uniform_int_distribution<> dist_tt(1, 240);
    for (int i = 0; i < totalVeiculos; ++i) {
        tt_values.push_back(dist_tt(gen));
    }

    std::vector<int> hd_values;
    std::vector<int> hn_values;
    for (int total_time : tt_values) {
        // 'hd' pode ser no máximo o tempo total.
        std::uniform_int_distribution<> dist_hd(0, total_time);
        int hd = dist_hd(gen);
        hd_values.push_back(hd);

        // 'hn' pode ser no máximo o tempo restante (tt - hd).
        int remaining_time = total_time - hd;
        std::uniform_int_distribution<> dist_hn(0, remaining_time);
        hn_values.push_back(dist_hn(gen));
    }

    outFile << "hd";
    for (int val : hd_values) { outFile << " " << val; }
    outFile << "\n";

    outFile << "hn";
    for (int val : hn_values) { outFile << " " << val; }
    outFile << "\n";
    
    outFile << "tt";
    for (int val : tt_values) { outFile << " " << val; }
    outFile << "\n";
    ...
    gerarLinha("km", 0, 4000);
    gerarLinha("tc", 0, 40);
    gerarLinha("mv", 0, 10);
    gerarLinha("mq", 0, 10);
    gerarLinha("mm", 0, 12);
    gerarLinha("av", 1980, 2024);

	const std::vector<int> potencias_validas = {
        60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 
        160, 170, 180, 190, 200, 225, 250, 275, 300
    };
	std::uniform_int_distribution<> dist_potencia_index(0, potencias_validas.size() - 1);
	outFile << "pv";
    for (int i = 0; i < totalVeiculos; ++i) {
        int randomIndex = dist_potencia_index(gen);
        outFile << " " << potencias_validas[randomIndex];
    }
    outFile << "\n";
